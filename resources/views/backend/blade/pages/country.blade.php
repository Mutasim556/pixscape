 ...
 @section('content')
     ...
     <div class="modal fade" id="add-country-modal" tabindex="-1" aria-labelledby="bs-example-modal-lg" aria-hidden="true">
         <div class="modal-dialog modal-lg">
             <div class="modal-content">
                 <div class="modal-header d-flex align-items-center" style="border-bottom:1px dashed gray">
                     <h4 class="modal-title" id="myLargeModalLabel">
                         {{ __('admin_local.Add Country Representation') }}
                     </h4>
                 ...
                 <div class="modal-body" style="margin-top: -20px">
                     <form method="POSt" action="" id="add_country_form" enctype="multipart/form-data">
                         @csrf
                         <div class="row">
     ...
     <div class="modal fade" id="edit-country-modal" tabindex="-1" aria-labelledby="bs-example-modal-lg"
         aria-hidden="true">
         <div class="modal-dialog modal-lg">
             <div class="modal-content">
                 <div class="modal-header d-flex align-items-center" style="border-bottom:1px dashed gray">
                     <h4 class="modal-title" id="myLargeModalLabel">
                         {{ __('admin_local.Edit Country Representation') }}
                     </h4>
                     <button type="button" class="btn-close " data-bs-dismiss="modal" aria-label="Close"></button>
                 </div>
                 <p class="px-3 text-danger">
                 ...
                 <div class="modal-body" style="margin-top: -20px">
                     <form id="edit_country_form" enctype="multipart/form-data">
                         @csrf
                         @method('PUT')
             ...
             <div class="col-lg-12 mx-auto">
                 <div class="card">
                     <div class="card-header py-3" style="border-bottom: 2px dashed gray">
                         <h3 class="card-title mb-0 text-center">{{ __('admin_local.countries') }}</h3>
                     </div>

                     <div class="card-body">
                         @if (hasPermission(['country-store']))
                             <div class="row mb-3">
                                 <div class="col-md-3">
                                     <button class="btn btn-success" type="btn" data-bs-toggle="modal"
                                         data-bs-target="#add-country-modal">+
                                         {{ __('admin_local.Add Country Representation') }}</button>
                                 </div>
                             </div>
                         @endif
                             ...
                             <table id="basic-1" class="display table-bordered">
                                 ...
                                 <tbody>
                                     @foreach ($countries as $country)
                                         <tr id="trid-{{ $country->id }}" data-id="{{ $country->id }}">
                                             <td>
                                                 @if ($country->logo)
                                                     <img height="50px" src="{{ asset($country->logo) }}"
                                                         alt="">
                                                 @else
                                             ...
                                             <td class="text-center">
                                                 @if (hasPermission(['country-update']))
                                                     <span
                                                         class="mx-2">{{ $country->status == 0 ? 'Inactive' : 'Active' }}</span><input
                                                         data-status="{{ $country->status == 0 ? 1 : 0 }}"
                                             ...
                                             <td>
                                                 @if (hasPermission(['country-update', 'country-delete']))
                                                     <div class="dropdown">
                                                         <button
 ...
 @push('js')
     ...
     <script>
         ...
         var form_url = "{{ route('admin.pages.country.store') }}";
     ...
     <script>
         var projPoint = `{{ __('admin_local.country Points') }}`;
         var defa = `{{ __('admin_local.Default') }}`;
     </script>
     <script src="{{ asset(env('ASSET_DIRECTORY', 'public') . '/' . 'admin/custom/country/country.js') }}"></script>
