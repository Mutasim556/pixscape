 ...
 @section('content')
     ...
     <div class="modal fade" id="add-award-modal" tabindex="-1" aria-labelledby="bs-example-modal-lg" aria-hidden="true">
         <div class="modal-dialog modal-lg">
             <div class="modal-content">
                 <div class="modal-header d-flex align-items-center" style="border-bottom:1px dashed gray">
                     <h4 class="modal-title" id="myLargeModalLabel">
                         {{ __('admin_local.Add award Option') }}
                     </h4>
                 ...
                 <div class="modal-body" style="margin-top: -20px">
                     <form method="POSt" action="" id="add_award_form" enctype="multipart/form-data">
                         @csrf
                         <div class="row">
     ...
     <div class="modal fade" id="edit-award-modal" tabindex="-1" aria-labelledby="bs-example-modal-lg"
         aria-hidden="true">
         <div class="modal-dialog modal-lg">
             <div class="modal-content">
                 <div class="modal-header d-flex align-items-center" style="border-bottom:1px dashed gray">
                     <h4 class="modal-title" id="myLargeModalLabel">
                         {{ __('admin_local.Edit awards') }}
                     </h4>
                     <button type="button" class="btn-close " data-bs-dismiss="modal" aria-label="Close"></button>
                 </div>
                 <p class="px-3 text-danger">
                 ...
                 <div class="modal-body" style="margin-top: -20px">
                     <form id="edit_award_form" enctype="multipart/form-data">
                         @csrf
                         @method('PUT')
                         <input type="hidden" id="award_id" name="award_id" value="">
                         <div class="row">
                             <div class="col-sm-12 col-xl-12">
                                 <ul class="nav nav-pills nav-primary my-0" id="pills-successtab" role="tablist">
                                     @php
             ...
             <div class="col-lg-10 mx-auto">
                 <div class="card">
                     <div class="card-header py-3" style="border-bottom: 2px dashed gray">
                         <h3 class="card-title mb-0 text-center">{{ __('admin_local.Our Awards') }}</h3>
                     </div>

                     <div class="card-body">
                         @if (hasPermission(['award-store']))
                             <div class="row mb-3">
                                 <div class="col-md-3">
                                     <button class="btn btn-success" type="btn" data-bs-toggle="modal"
                                         data-bs-target="#add-award-modal">+
                                         {{ __('admin_local.Add Award') }}</button>
                                 </div>
                             </div>
                         @endif
                             ...
                             <table id="basic-1" class="display table-bordered">
                                 ...
                                 <tbody>
                                     @foreach ($awards as $award)
                                         <tr id="trid-{{ $award->id }}" data-id="{{ $award->id }}">
                                             <td>{{ $award->title }}</td>
                                             <td>{!! $award->details !!}</td>

                                             <td class="text-center">
                                                 @if (hasPermission(['award-update']))
                                                     <span
                                                         class="mx-2">{{ $award->status == 0 ? 'Inactive' : 'Active' }}</span><input
                                                         data-status="{{ $award->status == 0 ? 1 : 0 }}"
                                                         id="status_change" type="checkbox" data-toggle="switchery"
                                                         data-color="green" data-secondary-color="red" data-size="small"
                                                         {{ $award->status == 1 ? 'checked' : '' }} />
                                                 @else
                                                     <span
                                             ...
                                             <td>
                                                 @if (hasPermission(['award-update', 'award-delete']))
                                                     <div class="dropdown">
                                                         <button
 ...
 @push('js')
     ...
     <script>
         ...
         var form_url = "{{ route('admin.pages.award.store') }}";
     ...
     <script src="{{ asset(env('ASSET_DIRECTORY', 'public') . '/' . 'admin/custom/award/award.js') }}"></script>
