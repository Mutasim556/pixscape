 ...
 @section('content')
     ...
     <div class="modal fade" id="add-notification-modal" tabindex="-1" aria-labelledby="bs-example-modal-lg" aria-hidden="true">
         <div class="modal-dialog modal-lg">
             <div class="modal-content">
                 <div class="modal-header d-flex align-items-center" style="border-bottom:1px dashed gray">
                     <h4 class="modal-title" id="myLargeModalLabel">
                         {{ __('admin_local.Add Notification') }}
                     </h4>
                 ...
                 <div class="modal-body" style="margin-top: -20px">
                     <form method="POSt" action="" id="add_notification_form" enctype="multipart/form-data">
                         @csrf
                         <div class="row">
                             <div class="col-sm-12 col-xl-6">
                                 <div class="row">
                                     <div class="form-group col-md-12">
                                         <label for="">{{ __('admin_local.Type') }}
                                             *</label>
                                 ...
                                 <div class="tab-content mt-3" id="pills-successtabContent">
                                     <div class="tab-pane fade show active" id="pills-defaultLang" role="tabpanel"
                                         ...
                                         <div class="form-group">
                                             <label for="">{{ __('admin_local.Notification') }} (
                                                 {{ __('admin_local.Default') }} ) *</label>
                                             <textarea class="form-control ckeditorappend" name="notification" id="notification"></textarea>
                                             <span class="text-danger err-mgs" id="notification_err"></span>
                                         </div>
                                     </div>
                                     <script>
                                         var langCode = [];
                                     ...
                                     @foreach (getLangs() as $lang)
                                         ...
                                         <div class="tab-pane fade" id="pills-{{ $lang->name }}" role="tabpanel"
                                             ...
                                             <div class="form-group">
                                                 <label for="">{{ __('admin_local.Notification') }} (
                                                     {{ $lang->name }} ) </label>
                                                 <textarea class="form-control" name="notification_{{ $lang->lang }}" id="notification_{{ $lang->lang }}"></textarea>
                                             </div>
                                         </div>
                                     @endforeach
                                 </div>
     ...
     <div class="modal fade" id="edit-notification-modal" tabindex="-1" aria-labelledby="bs-example-modal-lg"
         aria-hidden="true">
         <div class="modal-dialog modal-lg">
             <div class="modal-content">
                 <div class="modal-header d-flex align-items-center" style="border-bottom:1px dashed gray">
                     <h4 class="modal-title" id="myLargeModalLabel">
                         {{ __('admin_local.Edit notification') }}
                     </h4>
                     <button type="button" class="btn-close " data-bs-dismiss="modal" aria-label="Close"></button>
                 </div>
                 <p class="px-3 text-danger">
                 ...
                 <div class="modal-body" style="margin-top: -20px">
                     <form id="edit_notification_form" enctype="multipart/form-data">
                         @csrf
                         @method('PUT')
                                 ...
                                 <div class="tab-content mt-3" id="epills-successtabContent">
                                     <div class="tab-pane fade show active" id="epills-defaultLang" role="tabpanel"
                                         ...
                                         <div class="form-group">
                                             <label for="">{{ __('admin_local.Notification') }} (
                                                 {{ __('admin_local.Default') }} ) *</label>
                                             <textarea class="form-control ckeditorappend" name="notification" id="notification2"></textarea>
                                             <span class="text-danger err-mgs" id="notification_err"></span>
                                         </div>
                                     </div>
                                     <script>
                                     ...
                                     @foreach (getLangs() as $lang)
                                         ...
                                         <div class="tab-pane fade" id="epills-{{ $lang->name }}" role="tabpanel"
                                             ...
                                             <div class="form-group">
                                                 <label for="">{{ __('admin_local.Notification') }} (
                                                     {{ $lang->name }} ) </label>
                                                 <textarea class="form-control" name="notification_{{ $lang->lang }}"
                                                     id="notification2_{{ $lang->lang }}"></textarea>
                                             </div>
                                         </div>
                                     @endforeach
                                 </div>
             ...
             <div class="col-lg-12 mx-auto">
                 <div class="card">
                     <div class="card-header py-3" style="border-bottom: 2px dashed gray">
                         <h3 class="card-title mb-0 text-center">{{ __('admin_local.Notifications') }}</h3>
                     </div>

                     <div class="card-body">
                         @if (hasPermission(['notification-store']))
                             <div class="row mb-3">
                                 <div class="col-md-3">
                                     <button class="btn btn-success" type="btn" data-bs-toggle="modal"
                                         data-bs-target="#add-notification-modal">+
                                         {{ __('admin_local.Add Notification') }}</button>
                                 </div>
                             </div>
                         @endif
                         ...
                         <div class="table-responsive theme-scrollbar">
                             <table id="basic-1" class="display table-bordered">
                                 <thead>
                                     <tr>
                                         <th>{{ __('admin_local.Notification') }}</th>
                                 ...
                                 <tbody>
                                     @foreach ($notifications as $notification)
                                         <tr id="trid-{{ $notification->id }}" data-id="{{ $notification->id }}">
                                             <td>{!! $notification->notification !!}</td>
                                             <td>{{ $notification->type }}</td>
                                             <td class="text-center">
                                                 @if (hasPermission(['notification-update']))
                                                     <span
                                                         class="mx-2">{{ $notification->status == 0 ? 'Inactive' : 'Active' }}</span><input
                                                         data-status="{{ $notification->status == 0 ? 1 : 0 }}"
                                             ...
                                             <td>
                                                 @if (hasPermission(['notification-update', 'notification-delete']))
                                                     <div class="dropdown">
                                                         <button
 ...
 @push('js')
     ...
     @foreach (getLangs() as $lang)
         <script>
             CKEDITOR.replace('notification_' + '{{ $lang->lang }}', {
                 on: {
         ...
         <script>
             CKEDITOR.replace('notification2_' + '{{ $lang->lang }}', {
                 on: {
                     contentDom: function(evt) {
                         // Allow custom context menu only with table elemnts.
                         evt.editor.editable().on('contextmenu', function(contextEvent) {
     ...
     <script>
         ...
         CKEDITOR.replace('notification2', {
             on: {
                 contentDom: function(evt) {
                     // Allow custom context menu only with table elemnts.
                     evt.editor.editable().on('contextmenu', function(contextEvent) {
         ...
         var form_url = "{{ route('admin.pages.notification.store') }}";
     ...
     <script>
         var projPoint = `{{ __('admin_local.notification Points') }}`;
         var defa = `{{ __('admin_local.Default') }}`;
     </script>
     <script src="{{ asset(env('ASSET_DIRECTORY', 'public') . '/' . 'admin/custom/notification/notification.js') }}"></script>
