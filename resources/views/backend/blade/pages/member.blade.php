 ...
 @section('content')
     ...
     <div class="modal fade" id="add-memberof-modal" tabindex="-1" aria-labelledby="bs-example-modal-lg" aria-hidden="true">
         <div class="modal-dialog modal-lg">
             <div class="modal-content">
                 <div class="modal-header d-flex align-items-center" style="border-bottom:1px dashed gray">
                     <h4 class="modal-title" id="myLargeModalLabel">
                         {{ __('admin_local.Add Memberof') }}
                     </h4>
                 ...
                 <div class="modal-body" style="margin-top: -20px">
                     <form method="POSt" action="" id="add_memberof_form" enctype="multipart/form-data">
                         @csrf
                         <div class="row">
                             ...
                             <div class="col-sm-12 col-xl-6">
                                 <div class="row">
                                     <div class="form-group col-md-12 text-end">
                                         ...
                                         <img src="" id="prev_logo" alt="" width="200"
                                             height="150">
                                     </div>
                                 </div>
                             </div>
     ...
     <div class="modal fade" id="edit-memberof-modal" tabindex="-1" aria-labelledby="bs-example-modal-lg"
         aria-hidden="true">
         <div class="modal-dialog modal-lg">
             <div class="modal-content">
                 <div class="modal-header d-flex align-items-center" style="border-bottom:1px dashed gray">
                     <h4 class="modal-title" id="myLargeModalLabel">
                         {{ __('admin_local.Edit Memberof') }}
                     </h4>
                     <button type="button" class="btn-close " data-bs-dismiss="modal" aria-label="Close"></button>
                 </div>
                 <p class="px-3 text-danger">
                 ...
                 <div class="modal-body" style="margin-top: -20px">
                     <form id="edit_memberof_form" enctype="multipart/form-data">
                         @csrf
                         @method('PUT')
                         <input type="hidden" id="memberof_id" name="memberof_id" value="">
                         <div class="row">
                             <div class="col-sm-12 col-xl-12">
                                 <ul class="nav nav-pills nav-primary my-0" id="pills-successtab" role="tablist">
                                     @php
             ...
             <div class="col-lg-12 mx-auto">
                 <div class="card">
                     <div class="card-header py-3" style="border-bottom: 2px dashed gray">
                         <h3 class="card-title mb-0 text-center">{{ __('admin_local.Member Of') }}</h3>
                     </div>

                     <div class="card-body">
                         @if (hasPermission(['memberof-store']))
                             <div class="row mb-3">
                                 <div class="col-md-3">
                                     <button class="btn btn-success" type="btn" data-bs-toggle="modal"
                                         data-bs-target="#add-memberof-modal">+
                                         {{ __('admin_local.Add Memberof') }}</button>
                                 </div>
                             </div>
                         @endif
                         ...
                         <div class="table-responsive theme-scrollbar">
                             <table id="basic-1" class="display table-bordered">
                                 ...
                                 <tbody>
                                     @foreach ($members as $memberof)
                                         <tr id="trid-{{ $memberof->id }}" data-id="{{ $memberof->id }}">
                                             <td>
                                                 @if ($memberof->logo)
                                                     <img height="50px" src="{{ asset($memberof->logo) }}"
                                                         alt="">
                                                 @else
                                             ...
                                             <td class="text-center">
                                                 @if (hasPermission(['memberof-update']))
                                                     <span
                                                         class="mx-2">{{ $memberof->status == 0 ? 'Inactive' : 'Active' }}</span><input
                                                         data-status="{{ $memberof->status == 0 ? 1 : 0 }}"
                                                         id="status_change" type="checkbox" data-toggle="switchery"
                                                         data-color="green" data-secondary-color="red" data-size="small"
                                                         {{ $memberof->status == 1 ? 'checked' : '' }} />
                                                 @else
                                                     <span
                                             ...
                                             <td>
                                                 @if (hasPermission(['memberof-update', 'memberof-delete']))
                                                     <div class="dropdown">
                                                         <button
 ...
 @push('js')
     ...
     <script>
         ...
         var form_url = "{{ route('admin.pages.memberof.store') }}";
     ...
     <script>
         var projPoint = `{{ __('admin_local.memberof Points') }}`;
         var defa = `{{ __('admin_local.Default') }}`;
     </script>
     <script src="{{ asset(env('ASSET_DIRECTORY', 'public') . '/' . 'admin/custom/memberof/memberof.js') }}"></script>
