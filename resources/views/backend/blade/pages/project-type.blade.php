 ...
 @section('content')
     ...
     <div class="modal fade" id="add-projecttype-modal" tabindex="-1" aria-labelledby="bs-example-modal-lg" aria-hidden="true">
         <div class="modal-dialog modal-lg">
             <div class="modal-content">
                 <div class="modal-header d-flex align-items-center" style="border-bottom:1px dashed gray">
                     <h4 class="modal-title" id="myLargeModalLabel">
                         {{ __('admin_local.Add Project Type') }}
                     </h4>
                 ...
                 <div class="modal-body" style="margin-top: -20px">
                     <form method="POSt" action="" id="add_projecttype_form" enctype="multipart/form-data">
                         @csrf
                         <div class="row">
                             <div class="col-sm-12 col-xl-12">
                                 ...
                                 <div class="tab-content mt-3" id="pills-successtabContent">
                                     <div class="tab-pane fade show active" id="pills-defaultLang" role="tabpanel"
                                         ...
                                         <div class="form-group">
                                             <label for="">{{ __('admin_local.Short Description') }} (
                                                 {{ __('admin_local.Default') }} ) *</label>
                                         ...
                                         <div class="tab-pane fade" id="pills-{{ $lang->name }}" role="tabpanel"
                                             ...
                                             <div class="form-group">
                                                 ...
                                                 <textarea class="form-control"
                                                     name="short_description_{{ $lang->lang }}"
                                                     id="short_description_{{ $lang->lang }}"></textarea>
                                             </div>
                                         </div>
                                     @endforeach
                                 </div>
                             ...
                             <div class="col-sm-12 col-xl-6">
                                 <div class="row">
                                     <div class="form-group col-md-12">
                                         <label for="">{{ __('admin_local.Image') }}
                                             *</label>
                             ...
                             <div class="col-sm-12 col-xl-6">
                                 <div class="row">
                                     <div class="form-group col-md-12 text-end">
                                         ...
                                         <img src="" id="prev_logo" alt="" width="100%">
                                     </div>
                                 </div>
                             </div>
                         </div>
     ...
     <div class="modal fade" id="edit-projecttype-modal" tabindex="-1" aria-labelledby="bs-example-modal-lg"
         aria-hidden="true">
         <div class="modal-dialog modal-lg">
             <div class="modal-content">
                 <div class="modal-header d-flex align-items-center" style="border-bottom:1px dashed gray">
                     <h4 class="modal-title" id="myLargeModalLabel">
                         {{ __('admin_local.Edit Project Type') }}
                     </h4>
                     <button type="button" class="btn-close " data-bs-dismiss="modal" aria-label="Close"></button>
                 </div>
                 <p class="px-3 text-danger">
                 ...
                 <div class="modal-body" style="margin-top: -20px">
                     <form id="edit_projecttype_form" enctype="multipart/form-data">
                         @csrf
                         @method('PUT')
                         <input type="hidden" id="projecttype_id" name="projecttype_id" value="">
                         <div class="row">
                             <div class="col-sm-12 col-xl-12">
                                 <ul class="nav nav-pills nav-primary my-0" id="pills-successtab" role="tablist">
                                     @php
             ...
             <div class="col-lg-12 mx-auto">
                 <div class="card">
                     <div class="card-header py-3" style="border-bottom: 2px dashed gray">
                         <h3 class="card-title mb-0 text-center">{{ __('admin_local.Project Types') }}</h3>
                     </div>

                     <div class="card-body">
                         @if (hasPermission(['projecttype-store']))
                             <div class="row mb-3">
                                 <div class="col-md-3">
                                     <button class="btn btn-success" type="btn" data-bs-toggle="modal"
                                         data-bs-target="#add-projecttype-modal">+
                                         {{ __('admin_local.Add Project Type') }}</button>
                                 </div>
                             </div>
                         @endif
                             ...
                             <table id="basic-1" class="display table-bordered">
                                 <thead>
                                     <tr>
                                         ...
                                         <th>{{ __('admin_local.Short Description') }}</th>
                                 ...
                                 <tbody>
                                     @foreach ($projectTypes as $projecttype)
                                         <tr id="trid-{{ $projecttype->id }}" data-id="{{ $projecttype->id }}">
                                             <td>
                                                 @if ($projecttype->image)
                                                     <img height="50px" src="{{ asset($projecttype->image) }}"
                                                         alt="">
                                                 @else
                                             ...
                                             <td>{{ $projecttype->short_description }}</td>
                                             <td class="text-center">
                                                 @if (hasPermission(['projecttype-update']))
                                                     <span
                                                         class="mx-2">{{ $projecttype->status == 0 ? 'Inactive' : 'Active' }}</span><input
                                                         data-status="{{ $projecttype->status == 0 ? 1 : 0 }}"
                                                         id="status_change" type="checkbox" data-toggle="switchery"
                                                         data-color="green" data-secondary-color="red" data-size="small"
                                                         {{ $projecttype->status == 1 ? 'checked' : '' }} />
                                                 @else
                                                     <span
                                             ...
                                             <td>
                                                 @if (hasPermission(['projecttype-update', 'projecttype-delete']))
                                                     <div class="dropdown">
                                                         <button
 ...
 @push('js')
     ...
     <script>
         ...
         var form_url = "{{ route('admin.pages.project-type.store') }}";
     ...
     <script>
         var projPoint = `{{ __('admin_local.projecttype Points') }}`;
         var defa = `{{ __('admin_local.Default') }}`;
     </script>
     <script src="{{ asset(env('ASSET_DIRECTORY', 'public') . '/' . 'admin/custom/project/project-type.js') }}"></script>
