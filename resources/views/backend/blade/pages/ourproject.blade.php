 ...
 @section('content')
     ...
     <div class="modal fade" id="add-ourproject-modal" tabindex="-1" aria-labelledby="bs-example-modal-lg" aria-hidden="true">
         <div class="modal-dialog modal-lg">
             <div class="modal-content">
                 <div class="modal-header d-flex align-items-center" style="border-bottom:1px dashed gray">
                     <h4 class="modal-title" id="myLargeModalLabel">
                         {{ __('admin_local.Add Our Project') }}
                     </h4>
                 ...
                 <div class="modal-body" style="margin-top: -20px">
                     <form method="POSt" action="" id="add_ourproject_form" enctype="multipart/form-data">
                         @csrf
                         <div class="row">
                             ...
                             <div class="col-sm-12 col-xl-6">
                                 <div class="row">
                                     <div class="form-group col-md-12">
                                         <label for="">{{ __('admin_local.Logo') }}
                                             *</label>
                                         <input type="file" class="form-control" name="logo" id="logo"
                                             accept="image/*"
                                             onchange="document.getElementById('prev_logo').src = window.URL.createObjectURL(this.files[0])">
                                         <span class="text-danger err-mgs" id="logo_err"></span>
                                     </div>
                             ...
                             <div class="col-sm-12 col-xl-6">
                                 <div class="row">
                                     <div class="form-group col-md-12 text-end">
                                         <label for="">{{ __('admin_local.Preview Logo') }} *</label><br>
                                         <img src="" id="prev_logo" alt="" width="200" height="150">
                                     </div>
                                 </div>
                             </div>
                         </div>
     ...
     <div class="modal fade" id="edit-ourproject-modal" tabindex="-1" aria-labelledby="bs-example-modal-lg"
         aria-hidden="true">
         <div class="modal-dialog modal-lg">
             <div class="modal-content">
                 <div class="modal-header d-flex align-items-center" style="border-bottom:1px dashed gray">
                     <h4 class="modal-title" id="myLargeModalLabel">
                         {{ __('admin_local.Edit Our Project') }}
                     </h4>
                     <button type="button" class="btn-close " data-bs-dismiss="modal" aria-label="Close"></button>
                 </div>
                 <p class="px-3 text-danger">
                 ...
                 <div class="modal-body" style="margin-top: -20px">
                     <form id="edit_ourproject_form" enctype="multipart/form-data">
                         @csrf
                         @method('PUT')
                         <input type="hidden" id="ourproject_id" name="ourproject_id" value="">
                         <div class="row">
                             <div class="col-sm-12 col-xl-12">
                                 <ul class="nav nav-pills nav-primary my-0" id="pills-successtab" role="tablist">
                                     @php
                         ...
                         <div class="row">
                             ...
                             <div class="col-sm-12 col-xl-6">
                                 <div class="row">
                                     <div class="form-group col-md-12">
                                         ...
                                         <input type="file" class="form-control" name="logo" id="logo"
                                             accept="image/*"
                                             onchange="document.getElementById('eprev_logo').src = window.URL.createObjectURL(this.files[0])">
                                         <span class="text-danger err-mgs" id="logo_err"></span>
                                     </div>
                             ...
                             <div class="col-sm-12 col-xl-6">
                                 <div class="row">
                                     <div class="form-group col-md-12 text-end">
                                         <label for="">{{ __('admin_local.Preview Logo') }} *</label><br>
                                         <img src="" id="eprev_logo" alt="" width="200"
                                             height="150">
                                     </div>
                                 </div>
                             </div>
             ...
             <div class="col-lg-12 mx-auto">
                 <div class="card">
                     <div class="card-header py-3" style="border-bottom: 2px dashed gray">
                         <h3 class="card-title mb-0 text-center">{{ __('admin_local.Our Projects') }}</h3>
                     </div>

                     <div class="card-body">
                         @if (hasPermission(['our-project-store']))
                             <div class="row mb-3">
                                 <div class="col-md-3">
                                     <button class="btn btn-success" type="btn" data-bs-toggle="modal"
                                         data-bs-target="#add-ourproject-modal">+
                                         {{ __('admin_local.Add Our Project') }}</button>
                                 </div>
                             </div>
                         @endif
                             ...
                             <table id="basic-1" class="display table-bordered">
                                 ...
                                 <tbody>
                                     @foreach ($ourprojects as $ourproject)
                                         <tr id="trid-{{ $ourproject->id }}" data-id="{{ $ourproject->id }}">
                                             <td>
                                                 @if ($ourproject->logo)
                                                     <img height="50px" src="{{ asset($ourproject->logo) }}" alt="">
                                                 @else
                                                     {{ __('admin_local.No File') }}
                                                 @endif
                                             </td>
                                             <td>{{ $ourproject->name }}</td>
                                             <td>{{ $ourproject->link }}</td>
                                             <td class="text-center">
                                                 @if (hasPermission(['ourproject-update']))
                                                     <span
                                                         class="mx-2">{{ $ourproject->status == 0 ? 'Inactive' : 'Active' }}</span><input
                                                         data-status="{{ $ourproject->status == 0 ? 1 : 0 }}"
                                                         id="status_change" type="checkbox" data-toggle="switchery"
                                                         data-color="green" data-secondary-color="red" data-size="small"
                                                         {{ $ourproject->status == 1 ? 'checked' : '' }} />
                                                 @else
                                                     <span
 ...
 @push('js')
     ...
     <script>
         ...
         var oTable = $("#basic-1").DataTable({
             "language": {
                 ...
                 "info": "Shoourproject _START_ to _END_ of _TOTAL_ entries",
                 "infoEmpty": "Shoourproject 0 to 0 of 0 entries",
         ...
         var form_url = "{{ route('admin.pages.our-projects.store') }}";
     ...
     <script src="{{ asset(env('ASSET_DIRECTORY', 'public') . '/' . 'admin/custom/ourproject/ourproject.js') }}"></script>
