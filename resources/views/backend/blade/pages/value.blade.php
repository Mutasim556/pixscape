 ...
 @section('content')
     ...
     <div class="modal fade" id="add-value-modal" tabindex="-1" aria-labelledby="bs-example-modal-lg" aria-hidden="true">
         <div class="modal-dialog modal-lg">
             <div class="modal-content">
                 <div class="modal-header d-flex align-items-center" style="border-bottom:1px dashed gray">
                     <h4 class="modal-title" id="myLargeModalLabel">
                         {{ __('admin_local.Add value Option') }}
                     </h4>
                 ...
                 <div class="modal-body" style="margin-top: -20px">
                     <form method="POSt" action="" id="add_value_form" enctype="multipart/form-data">
                         @csrf
                         <div class="row">
     ...
     <div class="modal fade" id="edit-value-modal" tabindex="-1" aria-labelledby="bs-example-modal-lg"
         aria-hidden="true">
         <div class="modal-dialog modal-lg">
             <div class="modal-content">
                 <div class="modal-header d-flex align-items-center" style="border-bottom:1px dashed gray">
                     <h4 class="modal-title" id="myLargeModalLabel">
                         {{ __('admin_local.Edit values') }}
                     </h4>
                     <button type="button" class="btn-close " data-bs-dismiss="modal" aria-label="Close"></button>
                 </div>
                 <p class="px-3 text-danger">
                 ...
                 <div class="modal-body" style="margin-top: -20px">
                     <form id="edit_value_form" enctype="multipart/form-data">
                         @csrf
                         @method('PUT')
             ...
             <div class="col-lg-10 mx-auto">
                 <div class="card">
                     <div class="card-header py-3" style="border-bottom: 2px dashed gray">
                         <h3 class="card-title mb-0 text-center">{{ __('admin_local.Our Values') }}</h3>
                     </div>

                     <div class="card-body">
                         @if (hasPermission(['value-store']))
                             <div class="row mb-3">
                                 <div class="col-md-3">
                                     <button class="btn btn-success" type="btn" data-bs-toggle="modal"
                                         data-bs-target="#add-value-modal">+
                                         {{ __('admin_local.Add Value') }}</button>
                                 </div>
                             </div>
                         @endif
                             ...
                             <table id="basic-1" class="display table-bordered">
                                 ...
                                 <tbody>
                                     @foreach ($values as $value)
                                         <tr id="trid-{{ $value->id }}" data-id="{{ $value->id }}">
                                             <td>{{ $value->title }}</td>
                                             <td>{!! $value->details !!}</td>
                                             ...
                                             <td class="text-center">
                                                 @if (hasPermission(['value-update']))
                                                     <span
                                                         class="mx-2">{{ $value->status == 0 ? 'Inactive' : 'Active' }}</span><input
                                                         data-status="{{ $value->status == 0 ? 1 : 0 }}"
                                             ...
                                             <td>
                                                 @if (hasPermission(['value-update', 'value-delete']))
                                                     <div class="dropdown">
                                                         <button
                                                         ...
                                                         <div class="dropdown-content">
                                                             @if (hasPermission(['value-update']))
                                                                 <a data-bs-toggle="modal" style="cursor: pointer;"
                                                                     data-bs-target="#edit-value-modal"
 ...
 @push('js')
     ...
     <script>
         ...
         var form_url = "{{ route('admin.pages.values.store') }}";
     ...
     <script src="{{ asset(env('ASSET_DIRECTORY', 'public') . '/' . 'admin/custom/values/values.js') }}"></script>
