$(document).on('submit', '#add_country_form', function (e) {
    ...
    $.ajax({
        ...
        success: function (rdata) {
            $('button[type=submit]', '#add_country_form').html(submit_btn_before);
            $('button[type=submit]', '#add_country_form').removeClass('disabled');
            swal({
                icon: "success",
                title: rdata.title,
                text: rdata.text,
                ...
                let countryM_image = data.logo ? '<img style="height: 50px;width:50px;" src="' + base_url + '/' + data.logo + '">' : no_file;


                $('#basic-1 tbody').append(`<tr id="trid-${data.id}" data-id="${data.id}"><td>${countryM_image}</td><td>${data.name}</td>
                ...
                $('#add_country_form .err-mgs').each(function (id, val) {
                    $(this).prev('input').removeClass('border-danger is-invalid')
                    $(this).prev('textarea').removeClass('border-danger is-invalid')
                    $(this).empty();
                })
                $('#add_country_form').trigger('reset');
                $('button[type=button]', '#add_country_form').click();
            })
        },
        error: function (err) {
            $('button[type=submit]', '#add_country_form').html(submit_btn_before);
            ...
            $.each(err.responseJSON.errors, function (idx, val) {
                ...
                $('#add_country_form #' + exp).addClass('border-danger is-invalid')
                $('#add_country_form #' + exp2).addClass('border-danger is-invalid')
                $('#add_country_form #' + exp).next('span').find('.select2-selection--single').attr('id', 'invalid-selec2')
                $('#add_country_form #' + exp).next('.err-mgs').empty().append(val);

                $('#add_country_form #' + exp + "_err").empty().append(val);
            })
        },
    })
});
...
$(document).on('change', '#status_change', function () {
    ...
    $.ajax({
        type: "get",
        url: 'country/update/status/' + update_id + "/" + status,
...
$(document).on('click', '#edit_button', function () {
    $('#edit_country_form').trigger('reset');
    $('#edit_country_form .err-mgs').each(function (id, val) {
        $(this).prev('input').removeClass('border-danger is-invalid')
        $(this).prev('textarea').removeClass('border-danger is-invalid')
        $(this).empty();
    })
    ...
    $.ajax({
        type: "get",
        url: 'country/' + cat + "/edit",
        dataType: 'JSON',
        headers: {
            'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
        },
        success: function (data) {
            $('#edit_country_form #country_id').val(data.id);
            $('#edit_country_form #name').val(data.name);

            $('#edit_country_form #eprev_logo').prop('src', base_url + "/" + data.logo);
            $.each(data.translations, function (key, val) {
                if (val.locale == 'en') {
                } else {
                    if (val.key == 'name') {
                        $('#edit_country_form #name_' + val.locale).val(val.value);
                    }
                }
            })
        },
        error: function (err) {
            if (err.status === 403) {
                ...
                swal({
                    ...
                    $('button[type=button]', '#edit_country_form').click();
                });
...
$('#edit_country_form').submit(function (e) {
    ...
    $('button[type=submit]', this).html(submit_btn_after + '....');
    $('button[type=submit]', this).addClass('disabled');
    var trid = '#trid-' + $('#country_id', this).val();
    ...
    $.ajax({
        type: "post",
        url: 'country/' + $('#country_id', '#edit_country_form').val(),
        data: formData,
        dataType: 'JSON',
        headers: {
            'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
        ...
        processData: false,
        success: function (data) {
            $('button[type=submit]', '#edit_country_form').html(submit_btn_before);
            $('button[type=submit]', '#edit_country_form').removeClass('disabled');

            $('td:nth-child(1)', trid).html(data.country.logo ? `<img style="height: 50px;" src="${base_url + '/' + data.country.logo}">` : no_file);
            $('td:nth-child(2)', trid).html(data.country.name);
        ...
        error: function (err) {
            ...
            $.each(err.responseJSON.errors, function (idx, val) {
                ...
                $('#edit_country_form #' + idx).addClass('border-danger is-invalid')
                $('#edit_country_form #' + idx).next('.err-mgs').empty().append(val);
            })
        }
    });
});
