$(document).on('submit', '#add_memberof_form', function (e) {
    e.preventDefault();
    ...
    $.ajax({
        ...
        success: function (rdata) {
            $('button[type=submit]', '#add_memberof_form').html(submit_btn_before);
            $('button[type=submit]', '#add_memberof_form').removeClass('disabled');
            swal({
                icon: "success",
                title: rdata.title,
                text: rdata.text,
                ...
                let memberofM_image = data.logo ? '<img style="height: 50px;width:50px;" src="' + base_url + '/' + data.logo + '">' : no_file;


                $('#basic-1 tbody').append(`<tr id="trid-${data.id}" data-id="${data.id}"><td>${memberofM_image}</td><td>${data.name}</td>
                ...
                $('#add_memberof_form .err-mgs').each(function (id, val) {
                    $(this).prev('input').removeClass('border-danger is-invalid')
                    $(this).prev('textarea').removeClass('border-danger is-invalid')
                    $(this).empty();
                })
                $('#add_memberof_form').trigger('reset');
                $('button[type=button]', '#add_memberof_form').click();
            })
        },
        error: function (err) {
            $('button[type=submit]', '#add_memberof_form').html(submit_btn_before);
            ...
            $.each(err.responseJSON.errors, function (idx, val) {
                ...
                $('#add_memberof_form #' + exp).addClass('border-danger is-invalid')
                $('#add_memberof_form #' + exp2).addClass('border-danger is-invalid')
                $('#add_memberof_form #' + exp).next('span').find('.select2-selection--single').attr('id', 'invalid-selec2')
                $('#add_memberof_form #' + exp).next('.err-mgs').empty().append(val);

                $('#add_memberof_form #' + exp + "_err").empty().append(val);
            })
        },
    })
});
...
$(document).on('change', '#status_change', function () {
    ...
    $.ajax({
        type: "get",
        url: 'memberof/update/status/' + update_id + "/" + status,
...
$(document).on('click', '#edit_button', function () {
    $('#edit_memberof_form').trigger('reset');
    $('#edit_memberof_form .err-mgs').each(function (id, val) {
        $(this).prev('input').removeClass('border-danger is-invalid')
        $(this).prev('textarea').removeClass('border-danger is-invalid')
        $(this).empty();
    })
    ...
    $.ajax({
        type: "get",
        url: 'memberof/' + cat + "/edit",
        dataType: 'JSON',
        headers: {
            'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
        },
        success: function (data) {
            $('#edit_memberof_form #memberof_id').val(data.id);
            $('#edit_memberof_form #name').val(data.name);

            $('#edit_memberof_form #eprev_logo').prop('src', base_url + "/" + data.logo);
            $.each(data.translations, function (key, val) {
                if (val.locale == 'en') {
                } else {
                    if (val.key == 'name') {
                        $('#edit_memberof_form #name_' + val.locale).val(val.value);
                    }
                }
            })
        },
        error: function (err) {
            if (err.status === 403) {
                ...
                swal({
                    ...
                    $('button[type=button]', '#edit_memberof_form').click();
                });
...
$('#edit_memberof_form').submit(function (e) {
    e.preventDefault();
    $('button[type=submit]', this).html(submit_btn_after + '....');
    $('button[type=submit]', this).addClass('disabled');
    var trid = '#trid-' + $('#memberof_id', this).val();
    ...
    $.ajax({
        type: "post",
        url: 'memberof/' + $('#memberof_id', '#edit_memberof_form').val(),
        data: formData,
        dataType: 'JSON',
        headers: {
            'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
        ...
        processData: false,
        success: function (data) {
            $('button[type=submit]', '#edit_memberof_form').html(submit_btn_before);
            $('button[type=submit]', '#edit_memberof_form').removeClass('disabled');

            $('td:nth-child(1)', trid).html(data.member.logo ? `<img style="height: 50px;" src="${base_url + '/' + data.member.logo}">` : no_file);
            $('td:nth-child(2)', trid).html(data.member.name);
        ...
        error: function (err) {
            ...
            $.each(err.responseJSON.errors, function (idx, val) {
                ...
                $('#edit_memberof_form #' + idx).addClass('border-danger is-invalid')
                $('#edit_memberof_form #' + idx).next('.err-mgs').empty().append(val);
            })
        }
    });
});
