$(document).on('submit', '#add_notification_form', function (e) {
    e.preventDefault();
    ...
    $.ajax({
        ...
        success: function (rdata) {
            $('button[type=submit]', '#add_notification_form').html(submit_btn_before);
            $('button[type=submit]', '#add_notification_form').removeClass('disabled');
            swal({
                icon: "success",
                title: rdata.title,
                text: rdata.text,
                ...
                $('#basic-1 tbody').append(`<tr id="trid-${data.id}" data-id="${data.id}"><td>${data.notification}</td><td>${data.type}</td>
                ...
                $('#add_notification_form .err-mgs').each(function (id, val) {
                    $(this).prev('input').removeClass('border-danger is-invalid')
                    $(this).prev('textarea').removeClass('border-danger is-invalid')
                    $(this).empty();
                })
                $('#add_notification_form').trigger('reset');
                $('button[type=button]', '#add_notification_form').click();
            })
        },
        error: function (err) {
            $('button[type=submit]', '#add_notification_form').html(submit_btn_before);
            ...
            $.each(err.responseJSON.errors, function (idx, val) {
                ...
                $('#add_notification_form #' + exp).addClass('border-danger is-invalid')
                $('#add_notification_form #' + exp2).addClass('border-danger is-invalid')
                $('#add_notification_form #' + exp).next('span').find('.select2-selection--single').attr('id', 'invalid-selec2')
                $('#add_notification_form #' + exp).next('.err-mgs').empty().append(val);

                $('#add_notification_form #' + exp + "_err").empty().append(val);
            })
        },
    })
});
...
$(document).on('change', '#status_change', function () {
    ...
    $.ajax({
        type: "get",
        url: 'notification/update/status/' + update_id + "/" + status,
...
$(document).on('click', '#edit_button', function () {
    ...
    $('#edit_notification_form .err-mgs').each(function (id, val) {
        $(this).prev('input').removeClass('border-danger is-invalid')
        $(this).prev('textarea').removeClass('border-danger is-invalid')
        $(this).empty();
    })
    ...
    $.ajax({
        type: "get",
        url: 'notification/' + cat + "/edit",
        dataType: 'JSON',
        headers: {
            'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
        },
        success: function (data) {
            $('#edit_notification_form #notification_id').val(data.id);
            $('#edit_notification_form #type').val(data.type);

            CKEDITOR.instances['notification2'].setData(data.notification);
            $.each(data.translations, function (key, val) {
                if (val.locale == 'en') {
                } else {
                    if (val.key == 'notification') {
                        CKEDITOR.instances['notification2_' + val.locale].setData(val.value);
                    }
                }
            })
        },
        error: function (err) {
            if (err.status === 403) {
                ...
                swal({
                    ...
                    $('button[type=button]', '#edit_notification_form').click();
                });
...
$('#edit_notification_form').submit(function (e) {
    e.preventDefault();
    $('button[type=submit]', this).html(submit_btn_after + '....');
    $('button[type=submit]', this).addClass('disabled');
    var trid = '#trid-' + $('#notification_id', this).val();
    ...
    $.ajax({
        type: "post",
        url: 'notification/' + $('#notification_id', '#edit_notification_form').val(),
        data: formData,
        dataType: 'JSON',
        headers: {
            'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
        ...
        processData: false,
        success: function (data) {
            $('button[type=submit]', '#edit_notification_form').html(submit_btn_before);
            $('button[type=submit]', '#edit_notification_form').removeClass('disabled');
            $('td:nth-child(1)', trid).html(data.notification.notification);
            $('td:nth-child(2)', trid).html(`${data.notification.type}`);
        ...
        error: function (err) {
            ...
            $.each(err.responseJSON.errors, function (idx, val) {
                ...
                $('#edit_notification_form #' + idx).addClass('border-danger is-invalid')
                $('#edit_notification_form #' + idx).next('.err-mgs').empty().append(val);
            })
        }
    });
});
