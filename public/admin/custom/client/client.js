$(document).on('submit', '#add_client_form', function (e) {
    ...
    $.ajax({
        ...
        success: function (rdata) {
            $('button[type=submit]', '#add_client_form').html(submit_btn_before);
            $('button[type=submit]', '#add_client_form').removeClass('disabled');
            swal({
                icon: "success",
                title: rdata.title,
                text: rdata.text,
                ...
                let clientM_image = data.client_image ? '<img style="height: 50px;width:50px;" src="' + base_url + '/' + data.client_image + '">' : no_file;


                $('#basic-1 tbody').append(`<tr id="trid-${data.id}" data-id="${data.id}"><td>${clientM_image}</td><td>${data.client_name}</td><td>${data.client_details}</td>
                ...
                $('#add_client_form .err-mgs').each(function (id, val) {
                    $(this).prev('input').removeClass('border-danger is-invalid')
                    $(this).prev('textarea').removeClass('border-danger is-invalid')
                    $(this).empty();
                })
                $('#add_client_form').trigger('reset');
                $('button[type=button]', '#add_client_form').click();
            })
        },
        error: function (err) {
            $('button[type=submit]', '#add_client_form').html(submit_btn_before);
            ...
            $.each(err.responseJSON.errors, function (idx, val) {
                ...
                $('#add_client_form #' + exp).addClass('border-danger is-invalid')
                $('#add_client_form #' + exp2).addClass('border-danger is-invalid')
                $('#add_client_form #' + exp).next('span').find('.select2-selection--single').attr('id', 'invalid-selec2')
                $('#add_client_form #' + exp).next('.err-mgs').empty().append(val);

                $('#add_client_form #' + exp + "_err").empty().append(val);
            })
        },
    })
});
...
$(document).on('change', '#status_change', function () {
    ...
    $.ajax({
        type: "get",
        url: 'client/update/status/' + update_id + "/" + status,
...
$(document).on('click', '#edit_button', function () {
    $('#edit_client_form').trigger('reset');
    $('#edit_client_form .err-mgs').each(function (id, val) {
        $(this).prev('input').removeClass('border-danger is-invalid')
        $(this).prev('textarea').removeClass('border-danger is-invalid')
        $(this).empty();
    })
    ...
    $.ajax({
        ...
        success: function (data) {
            $('#edit_client_form #client_id').val(data.id);
            $('#edit_client_form #client_name').val(data.client_name);

            CKEDITOR.instances['client_details2'].setData(data.client_details);

            $('#edit_client_form #eprev_client_image').prop('src', base_url + "/" + data.client_image);
            $.each(data.translations, function (key, val) {
                if (val.locale == 'en') {
                } else {
                    if (val.key == 'client_name') {
                        $('#edit_client_form #client_name_' + val.locale).val(val.value);
                    }
                    if (val.key == 'client_details') {
                        CKEDITOR.instances['client_details2_' + val.locale].setData(val.value);
                    }
                }
            })
        },
        error: function (err) {
            if (err.status === 403) {
                ...
                swal({
                    ...
                    $('button[type=button]', '#edit_client_form').click();
                });
...
$('#edit_client_form').submit(function (e) {
    ...
    $('button[type=submit]', this).html(submit_btn_after + '....');
    $('button[type=submit]', this).addClass('disabled');
    var trid = '#trid-' + $('#client_id', this).val();
    ...
    $.ajax({
        type: "post",
        url: 'client/' + $('#client_id', '#edit_client_form').val(),
        data: formData,
        dataType: 'JSON',
        headers: {
            'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
        ...
        processData: false,
        success: function (data) {
            $('button[type=submit]', '#edit_client_form').html(submit_btn_before);
            $('button[type=submit]', '#edit_client_form').removeClass('disabled');

            $('td:nth-child(1)', trid).html(data.client.client_image ? `<img style="height: 50px;" src="${base_url + '/' + data.client.client_image}">` : no_file);
            $('td:nth-child(2)', trid).html(data.client.client_name);
            $('td:nth-child(3)', trid).html(`${data.client.client_details}`);
        ...
        error: function (err) {
            ...
            $.each(err.responseJSON.errors, function (idx, val) {
                ...
                $('#edit_client_form #' + idx).addClass('border-danger is-invalid')
                $('#edit_client_form #' + idx).next('.err-mgs').empty().append(val);
            })
        }
    });
});
