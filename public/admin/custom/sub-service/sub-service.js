$(document).on('submit','#add_sub_service_form',function(e){
    e.preventDefault();
    ...
    $.ajax({
        ...
        success: function (rdata) {
            $('button[type=submit]', '#add_sub_service_form').html('Submit');
            $('button[type=submit]', '#add_sub_service_form').removeClass('disabled');
            swal({
                icon: "success",
                title: rdata.title,
                text: rdata.text,
        ...
        error: function (err) {
            ...
            if(err.status===403){
                ...
                swal({
                    ...
                    $('button[type=button]', '#add_sub_service_form').click();
                });

            }

            $('#add_sub_service_form .err-mgs').each(function(id,val){
            ...
            $.each(err.responseJSON.errors,function(idx,val){
                ...
                $('#add_sub_service_form #'+exp).addClass('border-danger is-invalid')
                $('#add_sub_service_form #'+exp2).addClass('border-danger is-invalid')
                $('#add_sub_service_form #'+exp).next('span').find('.select2-selection--single').attr('id','invalid-selec2')
                $('#add_sub_service_form #'+exp).next('.err-mgs').empty().append(val);

                $('#add_sub_service_form #'+exp+"_err").empty().append(val);
            })
        },
    })
});
...
$(document).on('change','#status_change',function(){
    ...
    $.ajax({
        type: "get",
        url: 'sub-service/update/status/'+update_id+"/"+status,
