$(document).on('submit', '#add_comments_form', function (e) {
    e.preventDefault();
    ...
    $.ajax({
        ...
        success: function (rdata) {
            $('button[type=submit]', '#add_comments_form').html(submit_btn_before);
            $('button[type=submit]', '#add_comments_form').removeClass('disabled');
            swal({
                icon: "success",
                title: rdata.title,
                text: rdata.text,
                ...
                let commentsM_image = data.image ? '<img style="height: 50px;width:50px;" src="' + base_url + '/' + data.image + '">' : no_file;


                $('#basic-1 tbody').append(`<tr id="trid-${data.id}" data-id="${data.id}"><td>${commentsM_image}</td><td>${data.name}</td><td>${data.designation}</td><td>${data.comments}</td>
                ...
                $('#add_comments_form .err-mgs').each(function (id, val) {
                    $(this).prev('input').removeClass('border-danger is-invalid')
                    $(this).prev('textarea').removeClass('border-danger is-invalid')
                    $(this).empty();
                })
                $('#add_comments_form').trigger('reset');
                $('button[type=button]', '#add_comments_form').click();
            })
        },
        error: function (err) {
            $('button[type=submit]', '#add_comments_form').html(submit_btn_before);
            ...
            $.each(err.responseJSON.errors, function (idx, val) {
                ...
                $('#add_comments_form #' + exp).addClass('border-danger is-invalid')
                $('#add_comments_form #' + exp2).addClass('border-danger is-invalid')
                $('#add_comments_form #' + exp).next('span').find('.select2-selection--single').attr('id', 'invalid-selec2')
                $('#add_comments_form #' + exp).next('.err-mgs').empty().append(val);

                $('#add_comments_form #' + exp + "_err").empty().append(val);
            })
        },
    })
});
...
$(document).on('click', '#edit_button', function () {
    $('#edit_comments_form').trigger('reset');
    $('#edit_comments_form .err-mgs').each(function (id, val) {
        $(this).prev('input').removeClass('border-danger is-invalid')
        $(this).prev('textarea').removeClass('border-danger is-invalid')
        $(this).empty();
    })
    let cat = $(this).closest('tr').data('id');
    $("#edit_comments_form")[0].reset();
    ...
    $.ajax({
        ...
        success: function (data) {
            $('#edit_comments_form #comments_id').val(data.id);
            $('#edit_comments_form #name').val(data.name);
            $('#edit_comments_form #designation').val(data.designation);
            CKEDITOR.instances['comments2'].setData(data.comments);

            $('#edit_comments_form #eprev_image').prop('src', base_url + "/" + data.image);
            $.each(data.translations, function (key, val) {
                if (val.locale == 'en') {
                } else {
                    if (val.key == 'name') {
                        $('#edit_comments_form #name_' + val.locale).val(val.value);
                    }
                    if (val.key == 'designation') {
                        $('#edit_comments_form #designation_' + val.locale).val(val.value);
                    }
                    if (val.key == 'comments') {
                        CKEDITOR.instances['comments2_' + val.locale].setData(val.value);
                    }
                }
            })
        },
        error: function (err) {
            if (err.status === 403) {
                ...
                swal({
                    ...
                    $('button[type=button]', '#edit_comments_form').click();
                });
...
$('#edit_comments_form').submit(function (e) {
    ...
    $('button[type=submit]', this).html(submit_btn_after + '....');
    $('button[type=submit]', this).addClass('disabled');
    var trid = '#trid-' + $('#comments_id', this).val();
    ...
    $.ajax({
        type: "post",
        url: 'comments/' + $('#comments_id', '#edit_comments_form').val(),
        data: formData,
        dataType: 'JSON',
        headers: {
            'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
        ...
        processData: false,
        success: function (data) {
            $('button[type=submit]', '#edit_comments_form').html(submit_btn_before);
            $('button[type=submit]', '#edit_comments_form').removeClass('disabled');

            $('td:nth-child(1)', trid).html(data.comments.image ? `<img style="height: 50px;" src="${base_url + '/' + data.comments.image}">` : no_file);
            $('td:nth-child(2)', trid).html(data.comments.name);
            $('td:nth-child(3)', trid).html(data.comments.designation);
            $('td:nth-child(4)', trid).html(`${data.comments.comments}`);
            swal({
                icon: "success",
                title: data.title,
                text: data.text,
        ...
        error: function (err) {
            ...
            $.each(err.responseJSON.errors, function (idx, val) {
                ...
                $('#edit_comments_form #' + idx).addClass('border-danger is-invalid')
                $('#edit_comments_form #' + idx).next('.err-mgs').empty().append(val);
            })
        }
    });
});
