    ...
    $.ajax({
        ...
        success: function (rdata) {
            ...
            swal({
                ...
                let partnerM_image = data.partner_image ? '<img style="height: 50px;width:50px;" src="' + base_url + '/' + data.partner_image + '">' : no_file;

                
                $('#basic-1 tbody').append(`<tr id="trid-${data.id}" data-id="${data.id}"><td>${partnerM_image}</td><td>${data.partner_name}</td><td>${data.partner_details}</td>
                ...
                $('#add_partner_form').trigger('reset');
        ...
        error: function (err) {
            ...
            $.each(err.responseJSON.errors, function (idx, val) {
                ...
                $('#add_partner_form #' + exp2).addClass('border-danger is-invalid')
...
$(document).on('change', '#status_change', function () {
    ...
    $.ajax({
        type: "get",
        url: 'partner/update/status/' + update_id + "/" + status,
...
$(document).on('click', '#edit_button', function () {
    $('#edit_partner_form').trigger('reset');
    ...
    $.ajax({
        type: "get",
        url: 'partner/' + cat + "/edit",
        dataType: 'JSON',
        headers: {
            'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
        },
        success: function (data) {
            $('#edit_partner_form #partner_id').val(data.id);
            $('#edit_partner_form #partner_name').val(data.partner_name);
            
            CKEDITOR.instances['partner_details2'].setData(data.partner_details);

            $('#edit_partner_form #eprev_partner_image').prop('src', base_url + "/" + data.partner_image);
            $.each(data.translations, function (key, val) {
                if (val.locale == 'en') {
                } else {
                    if (val.key == 'partner_name') {
                        $('#edit_partner_form #partner_name_' + val.locale).val(val.value);
                    }
                    if (val.key == 'partner_details') {
                        CKEDITOR.instances['partner_details2_' + val.locale].setData(val.value);
                    }
                }
            })
        },
...
$('#edit_partner_form').submit(function (e) {
    e.preventDefault();
    $('button[type=submit]', this).html(submit_btn_after + '....');
    $('button[type=submit]', this).addClass('disabled');
    var trid = '#trid-' + $('#partner_id', this).val();
    ...
    $.ajax({
        type: "post",
        url: 'partner/' + $('#partner_id', '#edit_partner_form').val(),
        data: formData,
        dataType: 'JSON',
        headers: {
            'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
        ...
        success: function (data) {
            ...
            $('td:nth-child(1)', trid).html(data.partner.partner_image ? `<img style="height: 50px;" src="${base_url + '/' + data.partner.partner_image}">` : no_file);
            $('td:nth-child(2)', trid).html(data.partner.partner_name);
            $('td:nth-child(3)', trid).html(`${data.partner.partner_details}`);
        ...
        error: function (err) {
            ...
            $.each(err.responseJSON.errors, function (idx, val) {
                ...
                $('#edit_partner_form #' + idx).addClass('border-danger is-invalid')
                $('#edit_partner_form #' + idx).next('.err-mgs').empty().append(val);
            })
        }
    });
});
